...
- Passwordless ssh access to the `Runtime` hosts **(recommended)**
  ...
  - [Install](#installation) mlcluster on the manager
  - Create the ssh configuration for each host to be used as Runtime by using the mlcluster CLI command `mlcluster add-runtime` as described [here](#add-host-to-ssh-config) and **do not forget** to specify the `--id-file` argument.
...
For a full list of CLI commands please use `mlcluster --help`. For the help of a specific command please use `mlcluster COMMAND --help`.
...
```bash
mlcluster add-runtime localhost root@localhost:22 --id_file ~/.ssh/id_rsa
```
...
```bash
mlcluster delete-runtime host-1
```
...
```python
from mlcluster import Runtime, RuntimeGroup
...
```python
from mlcluster import RuntimeManager, RuntimeGroup
...
Use different strategies for launching the master and the worker instance by providing custom implementation of `mlcluster.cluster.MasterLauncher` and `mlcluster.cluster.WorkerLauncher`. The default implementations are `mlcluster.cluster.dask_cluster.LocalMasterLauncher` and `mlcluster.cluster.dask_cluster.RoundRobinLauncher`.
...
- [MongoDB](https://docs.mongodb.com/manual/administration/install-on-linux/) server must be installed on the [manager](#manager).
  - **Note:** When using the [ml-workspace](https://github.com/khulnasoft/ml-workspace) as the `master` then you can use the provided install script for MongoDB which can be found under `/resources/tools`.
- [Hyperopt must be installed ](https://github.com/hyperopt/hyperopt) on all `Runtimes` where hyperopt workers will be started
  - **Note:** When using the [ml-workspace](https://github.com/khulnasoft/ml-workspace) as hosts for the `Runtimes` then hyperopt is already pre-installed.
...
```python
from mlcluster import RuntimeManager
from mlcluster.cluster.hyperopt_cluster import HyperoptCluster
...
In general you should read the [Logging, exception handling and debugging](#logging-exception-handling-and-debugging) section first so that you are aware of the general options mlcluster offers for debugging.<br/>
So the first step is to successfully launch a Hyperopt cluster by using the corresponding mlcluster class. If you experience problems until this point you should analyze the exceptions which should guide you forward to a solution. If this given error is not self explaining then please consider to provide meaningful feedback here so that it will be soon. Common problems until the cluster is started are:

- **MongoDB or hyperopt are not installed**, i.e. the prerequisites are not yet fulfilled.
  => Ensure that the prerequisites are fulfilled. Consider using [ml-workspace](https://github.com/khulnasoft/ml-workspace) to get rid of dependency problems.
...
Use different strategies for launching the master and the worker instances by providing custom implementation of `mlcluster.cluster.MasterLauncher` and `mlcluster.cluster.WorkerLauncher`. The default implementations are `mlcluster.cluster.hyperopt_cluster.LocalMongoLauncher` and `mlcluster.cluster.hyperopt_cluster.RoundRobinLauncher`.
...
```python
from mlcluster import RuntimeGroup
...
```python
from mlcluster import RuntimeTask, Runtime, RuntimeGroup, RuntimeManager
...
import pandas as pd
from mlcluster import RuntimeTask, RuntimeManager
...
We like to use the following basic configuration when using mlcluster in a [Jupyter](https://jupyter.org/) notebook:
...
```python
from mlcluster import RuntimeGroup, RuntimeTask
...
```python
from mlcluster import Runtime, RuntimeTask
...
task.run_command('echo mlcluster!')
...
print(task.execution_log[1]) # => 'mlcluster!'
